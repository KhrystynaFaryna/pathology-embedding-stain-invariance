...
def main():
  ...
  model = ResNet(dataset='2d-nxk', depth=18, num_classes=num_classes, bottleneck=True)
  print(model)
  model = model.cuda()
  ...
  if test_list is not None: 
    for test_set in test_list:
          ...
          train_queue, test1_queue = get_dataloaders(
            ...
            dataroot=args.dataroot, train_set=args.val_set, val_set=test_set+"_", dataset = args.dataset
            )
                ...
                if os.path.exists(data_m_n_dir):
                  ...
                  if len(time_stamp)==1:
                      ...
                      if 'weights.pt' in destination_folder_list:
                          ...
                          valid1_auc, valid1_obj = infer(test1_queue, model, criterion, args.save+args.train_set+time_stamp[0]+'.csv',test_set)
                    ...
                    for d in time_stamp:
                      ...
                      if 'weights.pt' in destination_folder_list:
                          ...
                          model_path=os.path.join(destination_folder,'weights.pt')
                          utils.load(model, model_path)
                          valid1_auc, valid1_obj = infer(test1_queue, model, criterion, args.save+args.train_set+d+'.csv',test_set)
                          print("auc:",valid1_auc)
                          utils.log_to_file(io_path = args.save+"/"+args.train_set+"log.csv", test_set = test_set, n=n, m=m,  valid_obj = valid1_obj, valid_metric = valid1_auc)
      ...
      train_queue, test1_queue = get_dataloaders(
        args.dataset, args.batch_size, args.num_workers,
        dataroot=args.dataroot, train_set=args.train_set, val_set=args.val_set1,dataset = args.dataset)
            ...
            if os.path.exists(data_m_n_dir):
              ...
              if len(time_stamp)==1:
                  ...
                  if 'weights.pt' in destination_folder_list:
                      ...
                      model_path=os.path.join(destination_folder,'weights.pt')
                      utils.load(model, model_path)
                      valid1_auc, valid1_obj = infer(test1_queue, model, criterion,args.save+args.train_set+time_stamp[0]+'.csv',args.val_set1)
                      print("auc:",valid1_auc)
                ...
                for d in time_stamp:
                  ...
                  if 'weights.pt' in destination_folder_list:
                      ...
                      model_path=os.path.join(destination_folder,'weights.pt')
                      utils.load(model, model_path)
                      valid1_auc, valid1_obj = infer(test1_queue, model, criterion,args.save+args.train_set+d+'.csv',args.val_set1)
                      print("auc:",valid1_auc)
                      utils.log_to_file(io_path = args.save+"/"+args.train_set+"log.csv", test_set = args.val_set1, n=n, m=m,  valid_obj = valid1_obj, valid_metric = valid1_auc)
      ...
      train_queue, test2_queue = get_dataloaders(
            args.dataset, args.batch_size, args.num_workers,
            dataroot=args.dataroot, train_set=args.train_set, val_set=args.val_set2,dataset = args.dataset)
            ...
            if os.path.exists(data_m_n_dir):
                ...
                if len(time_stamp)==1:
                  ...
                  if 'weights.pt' in destination_folder_list:
                      ...
                      model_path=os.path.join(destination_folder,'weights.pt')
                      utils.load(model, model_path)
                      valid2_auc, valid2_obj = infer(test2_queue, model, criterion,args.save+args.train_set+time_stamp[0]+'.csv',args.val_set2)
                  ...
                  for d in time_stamp:
                    ...
                    if 'weights.pt' in destination_folder_list:
                      ...
                      model_path=os.path.join(destination_folder,'weights.pt')
                      utils.load(model, model_path)
                      valid2_auc, valid2_obj = infer(test2_queue, model, criterion,args.save+args.train_set+d+'.csv',args.val_set2)
                      print("auc:",valid2_auc)
                      utils.log_to_file(io_path = args.save+args.train_set+"log.csv", test_set = args.val_set2, n=n, m=m, valid_obj = valid2_obj, valid_metric = valid2_auc)
                    else:
                      pass
...
def infer(valid_queue, model, criterion, save_file_name,dataset):
    ...
    prec1 = roc_auc_score(target_,pred_)            
