...
parser.add_argument('--net_version', type=str, default='2d-nxk', help='version of the network')
...
parser.add_argument('--randaugment_transforms_set', type=str, default='invariant_loop',choices=['review','midl2021','original','midl2021_tr2eb','midl2021_trsh2eb','manual','invariant', 'invariant_loop'],help='which set of randaugment transforms to use')
...
arg_dict['net_version']=args.net_version
arg_dict['fold']=args.fold

project_name='invariant'+args.val_set[3:-1]

time_stamp= args.dataset+'_'+args.val_set+'n_'+str(args.n)+'_m_'+str(args.m)+'_t_'+args.randaugment_transforms_set+"_k_"+str(args.k)+"_coef_"+str(args.coef)+'/'+str(args.fold)
wandb.init(project=project_name, name = args.val_set+'n_'+str(args.n)+'_m_'+str(args.m)+'_t_'+args.randaugment_transforms_set+"_k_"+str(args.k)+"_coef_"+str(args.coef),notes="first logging with wandb", 
...
def main():
  ...
  if os.path.exists(os.path.join(args.save, 'weights.pt')):
    utils.load(model, os.path.join(args.save, 'weights.pt'))
    epoch_restart = pd.read_csv(os.path.join(args.save, 'log.csv'))["epoch"].iloc[-1]
    lr_restart = pd.read_csv(os.path.join(args.save, 'log.csv'))["lr"].iloc[-1]
    if args.optimizer_type=='sdg':
      optimizer = torch.optim.SGD(model.parameters(),lr_restart,momentum=args.momentum,weight_decay=args.weight_decay)
    elif args.optimizer_type=='adam':
      optimizer = torch.optim.Adam(model.parameters(),lr_restart)
    elif args.optimizer_type=='rms':
      optimizer = torch.optim.RMSprop(model.parameters(),lr_restart,momentum=args.momentum,weight_decay=args.weight_decay)
    else:
      print('This optimizer is not implemented')
    for epoch in range(epoch_restart,args.epochs):
      #scheduler.step()
