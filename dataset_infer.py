...
class Camelyon17(Dataset):
    ...
    def __init__(self, x_path, y_path, transform=None, augmenter=None, test_classes=6 , dataset='camelyon17'):
        """
        Args:
        ...
        if self.dataset=="midog":
            ...
            if len(np.unique(self.y))>2:
                print('loaded data has', len(np.unique(self.y)),'classes')
                print('class 0',len(self.y[self.y==test_classes ]))
               
                
                self.targets=self.y[(self.y==test_classes)]

                print(self.targets.shape)
                self.x=self.x[self.y==test_classes ,:,:,:]
                print('x shape:',self.x.shape)
                print('Using only classes 0 and 1')
                print('modified to ', len(np.unique(self.targets)),'classes')
...
def get_dataloaders(dataset, batch, num_workers, dataroot, train_set, val_set, augmenter='hed-light', dataset="midog"):
    ...
    if dataset == 'camelyon17':
        ...
        val_y_path = dataroot+val_set+'y.npy'
        #Load train data
        total_trainset = Camelyon17(x_path, y_path, transform=None,augmenter = None, dataset='midog') # 4000 trainset
