...
from dataset_infer import get_dataloaders, num_class
...
'''
./c-submit --require-gpu-mem="8G" --gpu-count="1" --require-mem="65G" --require-cpus="8" --priority="high" khrystynafaryna 9548 72 doduo1.umcn.nl/khrystynafaryna/kf_container_rand:latest python3 /data/pathology/projects/autoaugmentation/from_chansey_review/randaugment/inference.py 
...
parser.add_argument('--dataset', type=str, default='tiger',choices=['camelyon17','none','tiger'],
                    help='location of the data corpus')
parser.add_argument('--train_set', type=str, default='val_A2_jb_', help='train file name')
parser.add_argument('--val_set1', type=str, default='jb_', help='val file name')
parser.add_argument('--val_set2', type=str, default='jb_', help='val file name')
...
parser.add_argument('--save', type=str, default='/data/pathology/projects/autoaugmentation/from_chansey_review/manual/results/', help='experiment name')
parser.add_argument('--load', type=str, default='/data/pathology/projects/autoaugmentation/from_chansey_review/manual/experiments/', help='experiment name')
...
parser.add_argument('--randaugment_transforms_set', type=str, default='manual',choices=['review','midl2021','original','midl2021_tr2eb','midl2021_trsh2eb','manual'],help='which set of randaugment transforms to use')
  ...
  if test_list is not None: 
    ...
    for test_set in test_list:
          ...
          train_queue, test1_queue = get_dataloaders(
            ...
            dataroot=args.dataroot, train_set=args.train_set, val_set=test_set+"_", 
            )
          ...
          for n in n_range:
              for m in m_range:
                  #load model
                if args.dataset=="tiger":
                  data_m_n_dir = os.path.join(args.load,args.dataset+'_'+args.train_set+'n_'+str(n)+'_m_'+str(m)+'_t_'+args.randaugment_transforms_set)
                elif args.dataset=="camelyon17":
                  data_m_n_dir = os.path.join(args.load,args.train_set+'n_'+str(n)+'_m_'+str(m)+'_t_'+args.randaugment_transforms_set)
                ...
                if os.path.exists(data_m_n_dir):
                  ...
                  if len(time_stamp)==1:
                      ...
                      if 'weights.pt' in destination_folder_list:
                          ...
                          valid1_auc, valid1_obj = infer(test1_queue, model, criterion)
                          if valid1_auc != None:
                              print("auc:",valid1_auc)

                              average_score.append(valid1_auc)
                              utils.log_to_file(io_path = args.save+"/"+'ave_acc'+args.train_set+"log.csv", test_set = test_set, n=n, m=m,  valid_obj = valid1_obj, valid_metric = valid1_auc)
                          else:
                              print("auc:",valid1_auc)
                              utils.log_to_file(io_path = args.save+"/"+'ave_acc'+args.train_set+"log.csv", test_set = test_set, n=n, m=m,  valid_obj = valid1_obj, valid_metric = valid1_auc)
    ...
    print("Average accuracy:",np.mean(average_score))      
  else:
      
      #load data 1
      train_queue, test1_queue = get_dataloaders(
        args.dataset, args.batch_size, args.num_workers,
        dataroot=args.dataroot, train_set=args.train_set, val_set=args.val_set1)
      ...
      train_queue, test2_queue = get_dataloaders(
            args.dataset, args.batch_size, args.num_workers,
            dataroot=args.dataroot, train_set=args.train_set, val_set=args.val_set2)
            ...
            if os.path.exists(data_m_n_dir):
                ...
                if len(time_stamp)==1:
                  ...
                  if 'weights.pt' in destination_folder_list:
                      ...
                      model_path=os.path.join(destination_folder,'weights.pt')
                      utils.load(model, model_path)
                      valid2_auc, valid2_obj = infer(test2_queue, model, criterion)
...
def infer(valid_queue, model, criterion):
    ...
    if len(target_)>0:       
      ...
      pred_[pred_>0.5]=1
      pred_[pred_<=0.5]=0
      prec1 = accuracy_score(target_,pred_)            
      return prec1, objs.avg
    else:
      prec1 = None
      objs.avg = None
      return prec1, objs.avg
